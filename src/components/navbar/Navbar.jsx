  ...
  return (
    <header className="absolute top-0 left-0 z-20 w-full ">
      <div className="container mx-auto flex justify-between items-center p-5">
        ...
        <button
          className={`fixed bottom-5 right-5 md:hidden bg-white/40 text-white p-3 rounded-full shadow-lg z-40 ${
        ...
        <nav className="hidden md:flex md:ml-auto md:mr-auto font-medium text-base text-gray-300 justify-center">
          {NavbarLink.map((e) => (
            <div
              key={e.id}
              href={e.link}
              className={`mx-3 hover:text-white hover:bg-[#50668340]/20 hover:rounded-2xl py-1 px-3 transition-all duration-250 ease-in-out ${
                activeLink === e.id
                  ? "text-white px-3 py-1 rounded-2xl bg-[#50668340]/20"
                  : "px-2 py-2 rounded-md text-gray-300"
              }`}
              onClick={() => {
                scrollTo(e.link);
                setActiveLink(e.id);
